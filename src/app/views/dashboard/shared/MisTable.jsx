...
import { getMISTableList2, getSauDataIncomingWAC } from "../../../camunda_redux/redux/action";
...
import { getCardDataMis1 } from "../../../camunda_redux/redux/action";
...
const MisTable = (props) => {
    const { setSelectionMode, setHeaderText, setLoadingBln, setLoad, loader } = props;
    const classes = chartStyle();
    const { loadsVal } = props;
    const dispatch = useDispatch();
    const columns = [
        { name: 'sauDisplayName', title: 'Branch/Directorate' },
        // { name: 'totalFilesPendingFiveSau', title: '< 10', },
        // { name: 'totalFilesPendingFiveTenSau', title: '10 - 20' },
        // { name: 'totalFilePendingTenDaysSau', title: '> 20' },
        // { name: 'totalSau', title: 'Total' },
        { name: 'totalFilesPendingFiveCau', title: '< 10' },
        { name: 'totalFilesPendingFiveTenCau', title: '10 - 20' },
        { name: 'totalFilePendingTenDaysCau', title: '> 20' },
    ...
    useEffect(() => {
        let temp = [...firstRow]
        // if (temp[0]) temp[0].hasItems = false;

        let groupName = sessionStorage.getItem('department')
        let sauId = 0;
        props.getCardDataMis1(groupName, sauId).then((res) => {
            // let obj = {
            //     // fileId: 13784,
            //     // hasItems: true,
            //     // lastUpdated: null,
            ...
            //     // robjectId: "0",
            //     // sauBranch: "7WG.HRC",
            //     sauDisplayName: "HRC BRANCH",
            //     // sauId: 13766,
            //     // sauParent: null,
            //     totalCau: 183,
            //     totalFilePendingTenDaysCau: 183,
            //     // totalFilePendingTenDaysSau: 145,
            //     totalFilesPendingFiveCau: 0,
            //     // totalFilesPendingFiveSau: 0,
            //     totalFilesPendingFiveTenCau: 0,
            //     // totalFilesPendingFiveTenSau: 0,
            //     // totalSau: 145
            // }
            // temp.push(res)
            // temp[0].fileId = '13784'
            // temp[0].sauId = '13766'
            // temp[0].robjectId = '0'
            // temp[0].hasItems = false
            // temp[0].sauParent = null
            temp.unshift(res)
            temp[0].parentId = 0
            // temp.unshift(obj)
            setRowData(temp)
            console.log("temp unshift check:", temp, rowData)
        })
    }, [firstRow, bln])
                ...
                .then((response) => {
                    try {
                        if (response !== undefined) {
                            ...
                            // debugger
                            // if(response[0])
                            setFirstRow(firstRow.concat(response[0]))
    ...
    const HighlightedCell = ({ column, value, row, style, colorProp, ...restProps }) => (
        <Table.Cell{...restProps} style={{ cursor: 'pointer', ...style, }
        } onClick={(e) => handleClick(e, value, column, row)}>
    ...
    const [defaultColumnWidths] = useState([
        ...
        { columnName: 'totalFilesPendingFiveCau', width: 145 },
        { columnName: 'totalFilesPendingFiveTenCau', width: 145 },
        { columnName: 'totalFilePendingTenDaysCau', width: 145 },
        { columnName: 'totalCau', width: 145 },
    ...
    const [columnBands] = useState([
        // {
        //     title: 'Section Files Pending Outside',
        //     children: [
        ...
        // },
        {
            // title: 'Other Files Pending In Section',
            title: 'Section Files Pending Outside',
            children: [
    ...
    const getDataFromTable = () => {
        { console.log("data chect rowData", rowData) }
        return (
...
export default connect(mapStateToProps, { setLoadData, getMISTableList2, getSauDataIncomingWAC, getCardDataMis1 })(MisTable);
